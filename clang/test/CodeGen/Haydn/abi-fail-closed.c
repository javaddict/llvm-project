...
// RUN: not %clang_cc1 -triple haydn-unknown-elf -emit-obj \
// RUN:   -o /dev/null %t/musttail.c 2>&1 | FileCheck %s --check-prefix=MUSTOBJ
...
// musttail is accepted as IR and the backend refuses it (no isReturn+isCall
// terminator). interrupt is not a Haydn attribute — do not invent CC_ISR.
...
// MUSTOBJ: {{unable to translate instruction: call|unable to lower|failed to lower}}
