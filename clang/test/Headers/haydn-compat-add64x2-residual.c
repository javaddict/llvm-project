...
//
// C4.2 / G-DSP-COMPAT: AE_ADD64X2_vector is permanent UNSUPPORTED.
// HiFi dual-64 lane-wise add (no cross-lane carry) has no Haydn map:
// ae_int64x2 is a single DR64 bag; a scalar i64 add is silent-wrong
// (cross-lane carry). Do not EMULATE or bag-alias dual-64. Default mode
// fail-closes; __HAYDN_ALLOW_INEXACT_AE keeps the transitional scalar
// body for NatureDSP -c only. Hexagon peer is fail-closed / feature-
// gated C surface, not a silent scalar alias. Research:
