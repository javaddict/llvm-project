...
> | Item | Evidence |
> |------|----------|
> | **CB-148** SMS asserts on CoreMark | **FIXED.** `MachinePipeliner.cpp:1117` aborted the compiler on `matrix_add_const`: its cheap path reasons "same base, lower offset first, therefore disjoint" and then asserts the target agrees. **Not a scale bug** — `TargetInstrInfo::areMemAccessesTriviallyDisjoint` is a virtual whose default returns false for *every* pair, and Haydn enabled `MachinePipeliner` at O2+ without overriding it, so an unimplemented hook was an abort rather than a loss of precision. Implemented in the standard shape (same single base, non-scalable, `LowOffset + LowWidth <= HighOffset`); Haydn's `getMemOperandsWithOffsetWidth` already returns byte offsets per § 5.18. **A precision win, not just an assert silencer**: `MachineInstr::mayAlias` is built on this hook, and over 250 torture files the real op count went 16269 → 16268 (one store eliminated) with bundles 14355 → 14341. The first regression test written for it was **vacuous** — a hand-written i32 loop compiled fine unfixed; what reaches the path is a *halfword* read-modify-write whose inner loop vectorizes to `<4 x i16>`. Test `cb148-pipeliner-mem-disjoint.ll`, verified to abort without the override. Found only because `run_full_gate.sh` was run for the first time — lit, ctest and torture all pass with the bug present. |
> | **CB-147** packing density | **FIXED.** The first member placed decided the bundle format for the whole cycle: the two product rows have disjoint slot sets, `tryAdd` walked alternatives by descending slot bit and never reconsidered, so anything with a P3x placement took the 3-entry form at once and locked out the ten logicals that have no P3x placement — the immediate-arithmetic family plus `SET_HWLOOP`, a wide immediate fitting only a 2-entry entry. Measured first: 1961 of 2507 solver rejections (78%) were greedy-only. `tryAdd` now re-solves the cycle before rejecting, with greedy kept as the fast path so every cycle it already packed is packed identically; rejections fell to 518 and greedy-only lockouts to zero. The re-solve MOVES already-placed members, whose alternate descriptors are already published, so the recognizer keeps `CurrentCycleMIs` and re-stamps them — 683 relocations over the corpus, not a corner case. Density 14622 → 14355 bundles (−1.83%), ops/bundle 1.1126 → 1.1333, real op count identical at 16269, finalizer splits 0. Verified by execution: ctest 226/226, torture 1417/0. Reproducer `CB147_ReSolveSeatsE2OnlyADDI32`; probe `haydn_pack_probe.py`. |
> | **CB-143** unit model migration | The slot model is gone: `SLOT0/1/2` deleted, `HAYDN_NUM_FU_BITS` = 7, 512 logicals retargeted per def, 31 unencodable defs removed and the 21 tests that assembled them moved to live mnemonics (`51c24ca3e2d7`, `144a622c6f8f`, `080486f5b000`), store/load overlap enforced (`3f5f9b21dc6f`). **Its residual was mis-described and does not exist**: `HaydnFuncUnitWrapper::conflict` has no unit axis in the path that runs — `buildCandidate` fills `Required` from `PlacementAlternative::FieldSlots`, which is entry POSITION — and unit exclusivity is already exact for the current cycle via `canTryAddProduct` / `CycleState::OccupiedUnits`. Three `X2MUL32` are refused, pinned by execution as `TwoMACUnitsRejectThird`. What is genuinely unmodelled is narrower: `conflict()` alone decides lookahead cycles and the PostPipeliner's `checkConflict`, and neither sees units. Superseded by CB-147. |
