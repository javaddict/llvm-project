...
> |----|-----|-------|-----------------|
> | **CB-159** | P3 | GISel CSE compile time | embench-iot-v2 `nettle-sha256.c` at -O2 spends 10+ minutes inside GlobalISel CSEInfo (forced-stack sample: `GISelCSEInfo::insertNode` / `CSEMIRBuilder::memoizeMI` DenseMap probing) — ctest `bundlesim_embench_v2_nettle-sha256` times out its 300 s budget on compile alone. PRE-EXISTING on both lines: the pre-CB-155 toolchain (llvm-project-mhyang build) also exceeds 240 s on the bare TU, and mhyang's own suite README ships it as the known 11/12 timeout. Not the CB-153a postmisched class — this is the big unrolled sha256 block feeding the CSE map at IRTranslator/legalize time. |
