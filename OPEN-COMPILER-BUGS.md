...
> |----|-----|-------|-----------------|
> | **CB-159** | P3→FIXED | sha256 -O2 | Two bugs, neither PostRA. (1) Legalizer hang on `verify_benchmark` SLP `icmp <8 x i8>` → `freeze <8 x i1>` → `bitcast i8`: `G_BUILD_VECTOR` of `<N x s1>` retried forever unless every use was `G_UNMERGE`. Pack s1 lanes for freeze/bitcast. (2) Wrong digest: SLP `lshr <2 x i32> %v, <i32 10, i32 3>` stayed on X2SRL32, which uses only lane 0 of the amount (both lanes >>10). Non-splat constant vector shifts now scalarize. `guest_exit=0` at -O2. |
...
> | **CB-161** | P2 | scheduler SFR write-port co-issue (backend fatal) | `intrin_x2x4_sfr_cmp` regression case fails at -O1/-O2 on clang 22.1.8 e1898dc4732d with a BACKEND FATAL (not ILLEGAL_INSTRUCTION): `Haydn bundle invariant violated ... structural inverse: chosen Format E members are not unit-injective`. Root cause visible in the fatal's own MI dump: the scheduler packs TWO SFR-writing members into one bundle — `BUNDLE ... implicit-def $d1, implicit-def $sfr, implicit killed $d1, implicit killed $d0, implicit $sfr, implicit $d2, implicit $d3` — a `x2mux32` (pred2 → x2int32 via SFR read-modify-write) co-issued with an `x4mux16` (pred4 → x4int16, also SFR). Issue law: SFR is 2R/**1W** — at most one live SFR write per bundle. Each mux alone compiles and runs clean at -O2 (verified individually: x2mux32 PASS 251 bundles, x4mux16 PASS 251 bundles); the pair in one BB triggers the packing. This is the exact hazard class CB-153's landed golden port-budget gate was supposed to close ("SFR live-writes" budget on the two commit surfaces) — either that gate misses the pred-mux lowering's implicit $sfr def, or the packing happens on a surface the gate does not cover. Repro (in-tree, self-contained, `<haydn.h>` only): BundleSim `benchmarks/haydn_intrin/repro_2026_08_21_sfr_dual_mux_unit_injective.c` — run from repo root: `./build/run_c benchmarks/haydn_intrin/repro_2026_08_21_sfr_dual_mux_unit_injective.c` (fatal at -O1/-O2; `-O0` PASSes, exit 0, 393 bundles). Tracking test: `bundlesim_reg_intrin_x2x4_sfr_cmp` ctest (was the CB-140 closure signal; re-fired as this new class on the rebuilt toolchain). |
