...
> |----|-----|-------|-----------------|
> | **CB-143** | **P1** | unit model migration | The ISA moved from per-slot capability to a **unit model**: slots carry no capability, each entry maps to exactly one of 7 named units, and no two entries in a bundle may share a unit. Bundles are **96-bit format E**. **Two pieces landed** (`6f37a306084f`, `83959145aafd`): all 512 logicals the database has are off `Slot*` itineraries and onto the generated `Unit_*` classes, derived per def because a format class does not determine a unit set — `Slot12_ALU` alone covered three of them; and `HAYDN_NUM_FU_BITS` went 3 → 10, which is what the post-RA scoreboard needed to see any unit at all (the unit FuncUnits sit at indices 3..9 behind the retired slots, so every `Unit_*` itinerary had been recording an EMPTY Required set). **Blocked from here**: deleting SLOT0/1/2 needs the 31 defs the database does not have to go, and deleting those breaks 21 tests that assemble the retired mnemonics — five MC files with 62 live directives, plus sixteen `.mir`. That is § 5.6 residue, not this. Still unmodelled beyond that: per-entry immediate width, the store/load aliasing rule, and the register-port budget as a bundle-wide matching rather than the pairwise approximation in `HaydnFuncUnitWrapper::conflict` |
