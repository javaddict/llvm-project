...
> |----|-----|-------|-----------------|
> | **CB-143** | P2 | unit model migration | **The slot model is gone from LLVM.** `SLOT0/1/2` are deleted, `HAYDN_NUM_FU_BITS` is 7, and the itineraries declare the seven format E units and nothing else. Landed in four steps (`6f37a306084f`, `83959145aafd`, `3dd56bf31023`): 512 logicals retargeted per def from `instruction_type_index.json`; the post-RA scoreboard widened to see units at all (it had been recording an EMPTY Required set for every instruction since the members moved, because the unit FuncUnits sat behind the slots); the 21 tests that still assembled the retired mnemonics moved to the live ones; and 31 unencodable defs deleted. **LD32/LD64 stay** — deleting them breaks `HaydnBundle::canAdd` and `HaydnInstrInfo.cpp` decides by name substring (`N.contains("LD64")`), a coupling recorded rather than guessed at. **Still open, and why it is P2 now**: the packer models unit exclusion and nothing else of B4's list — per-entry immediate width, the store/load aliasing rule, and a bundle-wide matching for the register-port budget instead of `HaydnFuncUnitWrapper::conflict`'s pairwise approximation (exact for two instructions, permissive for three) |
