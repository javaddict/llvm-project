...
> |----|-----|-------|-----------------|
> | **CB-146** | P2 | format E / alignment | **A function alignment that is not a multiple of 12 leaves undecodable bytes in `.text`.** `gcc-c-torture align-3.c` declares `__attribute__((aligned(256)))`; the linker pads between functions, the fill is zeros, and an all-zero parcel is not a bundle (`bit[2:0]` must be `0b111`). The BSP link fails on the parcel-size assertion; relaxing it just moves the failure to `DISASSEMBLER_FAILURE` over 13 `<unknown>` bundles. **No Bundle128 equivalent**: a 16-byte parcel divides every power-of-two alignment, and all-zero WAS a valid Bundle128 NOP — both gone. Needs a decision, not a patch: compiler-side alignment policy, or BundleSim tolerating inter-function padding. Sole regression in the 2026-08-12 torture run (1416 PASS / 1 FAIL vs the 08-05 baseline's 1417 / 0) |
