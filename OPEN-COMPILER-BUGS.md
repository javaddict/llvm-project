...
> |----|-----|-------|-----------------|
> | **CB-148** | P2 | SMS / mem-dep | **clang asserts compiling CoreMark.** `MachinePipeliner.cpp:1117`, `hasLoopCarriedMemDep`: ``TII->areMemAccessesTriviallyDisjoint(SrcMI, DstMI) && "What happened to the chain edge?"`` — the swing pipeliner built a memory chain edge between two accesses that `areMemAccessesTriviallyDisjoint` then calls disjoint, and the two disagree. Repro: `simulator/benchmarks/coremark_haydn/core_matrix.c` at **-O2** (`scripts/run_full_gate.sh`, or the preprocessed reproducer clang writes out). Haydn enables `MachinePipeliner`/SMS at O2+ (`HaydnTargetMachine.cpp:216`) and does **not** override `areMemAccessesTriviallyDisjoint`, so this runs on the generic implementation over `HaydnInstrInfo::getMemOperandsWithOffsetWidth` — the same routine § 5.18 already caught returning an element index where a byte offset was wanted. **PRE-EXISTING, not CB-147**: verified by reverting CB-147's three source files, rebuilding, and reproducing the identical assertion. It had never been seen because `run_full_gate.sh` (ctest + CoreMark + Dhrystone) had not been run — the recorded baseline was ctest and torture only. Nothing in lit, ctest or gcc-c-torture covers it: torture is -O3 and its loops do not reach this shape. |
