...
> | **CB-143** unit model migration | The slot model is gone: `SLOT0/1/2` deleted, `HAYDN_NUM_FU_BITS` = 7, 512 logicals retargeted per def, 31 unencodable defs removed and the 21 tests that assembled them moved to live mnemonics (`6f37a306084f`, `83959145aafd`, `3dd56bf31023`), store/load overlap enforced (`2cddbe292469`). **Its residual was mis-described and does not exist**: `HaydnFuncUnitWrapper::conflict` has no unit axis in the path that runs — `buildCandidate` fills `Required` from `PlacementAlternative::FieldSlots`, which is entry POSITION — and unit exclusivity is already exact for the current cycle via `canTryAddProduct` / `CycleState::OccupiedUnits`. Three `X2MUL32` are refused, pinned by execution as `TwoMACUnitsRejectThird`. What is genuinely unmodelled is narrower: `conflict()` alone decides lookahead cycles and the PostPipeliner's `checkConflict`, and neither sees units. Superseded by CB-147. |
