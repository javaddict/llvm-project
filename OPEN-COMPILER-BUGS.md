...
> |----|-----|-------|-----------------|
> | **CB-143** | P3 | unit model migration | **The slot model is gone and the packer axes are settled.** `SLOT0/1/2` deleted, `HAYDN_NUM_FU_BITS` = 7, 512 logicals retargeted per def, 31 unencodable defs removed and the 21 tests that assembled them moved to the live mnemonics (`6f37a306084f`, `83959145aafd`, `3dd56bf31023`). Of B4's three remaining axes, **two were already modelled and the entry was wrong about them** (`2cddbe292469`): per-entry immediate width is enforced by construction — the generator emits a member only where the instruction fits, so `ADDI32` has two placements and `ADD32` seven — and the register-port budget is bundle-wide already, because `HaydnFuncUnitWrapper::operator|=` accumulates. The third, **store/load overlap, was genuinely missing and is now enforced**: the ISA requires the compiler to prove disjointness or split the bundle, the hardware raises an exception otherwise, and nothing checked it. ~2% bundles. **Residual: CLOSED, and the way it was written here named a mechanism that is not there.** This line used to say "unit exclusivity is still pairwise in one direction (three instructions each needing {ALU1, ALU2} pass and cannot all issue)". Measured: (1) `HaydnFuncUnitWrapper::conflict` has **no unit axis at all** in the path that runs — `buildCandidate` fills `Required` from `PlacementAlternative::FieldSlots`, which is *entry position*, and `SlotBits`/`UnitBits` are separate spaces by construction (`HaydnBaseInfo.h`); the itinerary-unit path is the no-alts fallback only. (2) Unit exclusivity **is** exact for the current cycle, because `getHazardType` gates on `canTryAddProduct` and `CycleState::OccupiedUnits` records one unit per member — three `X2MUL32` are rejected, since its five placements hold only MAC0/MAC1. So there is no "three pass and cannot issue" case to fix. What is genuinely unmodelled is narrower than the old wording: `conflict()` alone decides lookahead cycles (`DeltaCycles != 0`) and `checkConflict` for the PostPipeliner, and neither sees units. Superseded by **CB-147**, which is the real and larger imprecision, and points the other way. |
> | **CB-147** | P3 | packing density | **The first instruction placed in a cycle silently chooses the bundle format for the whole cycle, with no lookahead — and that, not unit exclusivity, is what limits packing.** `ProductFormatRows` has two rows whose slot sets are disjoint (`{P20,P21}` vs `{P30,P31,P32}`), and `HaydnBundlePlan.h` states it: "no occupancy is covered by both rows". `tryAdd` walks alternatives in **descending** slot bit (P32>P31>P30>P21>P20) and commits the first that fits, so anything holding a P3x placement takes E3 immediately and E2 becomes unreachable for the rest of that cycle. Measured over the generated table (3578 members, 684 logicals, 11 distinct placement signatures): **150 instruction sequences pack fewer ops under greedy than a valid assignment admits**. Reproduce with `llvm/lib/Target/Haydn/utils/haydn_pack_probe.py`, which re-reads the generated members rather than restating them, so a regenerated layout re-measures instead of going stale; the reproducer in `HaydnBundleFormatSolverTest.cpp` is `CB147_E3FirstLocksOutE2OnlyADDI32`. The victims are **exactly 10 logicals** with no P3x placement at all, because a wide immediate only fits a 2-entry entry: `ADDI32`, `ADDI32S`, `ANDI32`, `MOVEI_H`, `MOVEI_L`, `ORI32`, `SET_HWLOOP`, `SUBI32`, `SUBI32S`, `XORI32`. That is the immediate-arithmetic family and both halves of a constant materialisation — common instructions, which is what makes this worth an entry. They can share a bundle only when scheduled **first** in their cycle, and are otherwise pushed to a bundle of their own. Consistent with emitted code: 250 torture files give 10784 E3 vs 3838 E2 bundles, 1.1126 ops/bundle, and 11988 of 14622 bundles holding exactly one real op. **Not a wrong bundle** — every bundle emitted is legal; the cost is density and the schedule's cost model. **Not a drive-by fix, and the obvious fix is unsafe**: making `tryAdd` re-solve the cycle would relocate already-placed members, and `commitPlacementForEmit` has *already* stamped each one's `AltDescs->setAlternateDescriptor(MI, MemberOpcode)` — a relocated member's stamp would go stale and emit a genuinely wrong bundle. Re-stamping needs `MachineInstr` identity inside `CycleState`, which runs into the standing "do not hold `MachineInstr *`" rule. The contained alternative is a scheduler preference — when the cycle is empty, issue the most format-constrained ready instruction first — which is a `HaydnPostRASchedStrategy` change and needs the full suite plus torture behind it. |
...
> | **CB-146 inter-function alignment fill** | A function alignment that is not a multiple of 12 makes the linker pad between functions, the fill is zeros, and objdump calls each 12-byte chunk `<unknown>` — refused in four places at once (objdump string search, dump parser, coverage walk, linker script). All four were Bundle128 assumptions: a 16-byte parcel divides every power-of-two alignment, and all-zero WAS a valid Bundle128 NOP. The coverage walk now looks at the BYTES — an uncovered gap passes only when every byte is zero — which is stronger, not weaker: a desynced decoder leaves real instruction bytes uncovered and is still refused, verified by corrupting a format indicator. gcc-c-torture -O3 back to **1417 PASS / 0 FAIL**, the 2026-08-05 baseline exactly. `simulator 417b0c2`; case `cb146_overaligned_function` |
