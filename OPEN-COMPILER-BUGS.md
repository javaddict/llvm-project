...
> | **CB-144** | P3 | GISel legalize | `extractelement`/`insertelement` on a `<N x i1>` with a VARIABLE index is unlegalizable: "unable to legalize instruction: `%4:_(s1) = G_EXTRACT_VECTOR_ELT %3:_(<2 x s1>)`". Haydn has no vector-of-i1 and nothing in the target scalarizes a variable index into a select chain. It was hidden behind `clampMaxNumElements(…, S1, 1)`, which could only have asserted (CB-130's construct) and is removed in `f1ce318e671d`. No observed failure — found by probing, not by a reproducer |
