...
>
> 
### 2026-08-17 — CB-155…158 provenance: mhyang's sim test corpus, first run against this toolchain

All four were exposed in one sweep: the BundleSim reunification (sim `mhyang`
@ b8cfe95) brought mhyang's new suites (haydn-codegen, haydn-abi, gcc-compat,
gcc-torture-struct/dg-struct, embench-iot-v2/dsp) onto this toolchain
(fork/haydn @ 583108abf5bb) for the first time — ctest went 245 → 641 and
these are the reds that survived triage. Board: 626 pass, 4 plans-machine
loud-SKIPs, 11 red = CB-155(1) + CB-158(3) + CB-156(1) + CB-157(1) +
4 XPASS + 1 feature gap. Two observations that are NOT bugs here:
...
- **`__int128` is a feature gap, not a miscompile**: their
  ...
  rejects it ("not supported on this target"). Whether Haydn grows i128
  support is a product decision, not a defect entry.
