...
; RUN: %python -c "from pathlib import Path; p=Path(r'%S/../../lib/Target/Haydn/haydn-rt/PRODUCT-IDENTITY.txt').read_text(); assert 'user-printf.c' in p and 'memset-2.c' in p and 'builtin-bitops-1.c' in p and 'strlen-5.c' in p and 'CODE_IMAGE_REJECT' in p and '4b6677f8bf87' in p and '28700d57' in p and 'T-ABI4' in p and 'T-ABI6' in p and 'T-ABI11' in p and 'T-ABI12' in p and 'G-ECOSYSTEM-CONSUMERS' in p and 'G-LIBRARY-COVERAGE' in p and 'G-DEBUG-OBSERVABILITY' in p and 'G-TEST-EVIDENCE' in p; s=Path(r'%S/../../lib/Target/Haydn/haydn-rt/SOFTFLOAT-CONTRACT.txt').read_text(); assert 'T-SF10' in s and 'yarpgen' in s and '.bak' in s and '28700d57' in s; w=Path(r'%S/../../lib/Target/Haydn/MCTargetDesc/HaydnELFObjectWriter.cpp').read_text(); assert 'static_assert(ELF::EM_HAYDN == 259' in w and 'KVX' in w and 'do not invent' in w.lower(); t=Path(r'%S/../../lib/Target/Haydn/TargetInfo/HaydnTargetInfo.cpp').read_text(); assert 'EM_HAYDN=259' in t and 'KVX' in t"
...
; classified library seats, torture CODE_IMAGE_REJECT residual, 4b6677f8bf87
