...
HaydnLegalizerInfo::HaydnLegalizerInfo(const HaydnSubtarget &ST) {
  ...
  getActionDefinitionsBuilder({G_LOAD, G_STORE})
      ...
      // MemoryTy s1) would otherwise livelock the legalizer — minScalar(0, S8)
      // widens the value to s8, custom truncs it back to s1 and rewrites the
      // MMO to s1, and the same query repeats forever (unbounded vreg/MMO
      // growth, no diagnostic). Sub-byte mem belongs to
      // lowerIfMemSizeNotByteSizePow2() below, which zero-extends the value
      // into a byte-sized store instead.
      .customIf(
          [](const LegalityQuery &Query) {
