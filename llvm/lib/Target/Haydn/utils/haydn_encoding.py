...
def emit_reloc_geometry(placements: list[dict]) -> str:
    ...
    The key is (FieldSize, entry count, entry index, mapping value). Every part
    is available where a relocation is applied: FieldSize comes from the
    relocation's own RelocFieldInfo, the entry count is header bit 3, the entry
    index follows from the relocation's offset within its bundle, and the
    mapping value is read out of the entry itself. That is what lets MC and lld
    share one answer -- neither of them knows the member.
    ...
    # A key whose immediate sits at more than one position cannot be resolved
    # from the image alone -- it would need the type code too. Those are
    # DROPPED rather than guessed, so a relocation that ever needs one misses
    # the table and the consumer reports an error. Silently patching the wrong
    # bits is the failure mode this whole table exists to prevent. In practice
    # only narrow fields collide (NOP's 4-bit imm); every 12- and 20-bit
    # immediate -- which is all a relocation can target -- resolves uniquely.
    ...
    out = [
        ...
        "// (FieldSize, entry count, entry index, mapping value). See",
        "// FORMAT-E-SWITCH-PLAN.md 5.8.",
        "//===----------------------------------------------------------------===//",
        "",
        "// FieldSize, EntryCount, EntryIndex, Mapping, BundleLsb",
        "HAYDN_RELOC_GEOM_ROW_LIST(",
    ]
    if dropped:
        out[-1:-1] = [
            "// Ambiguous, deliberately absent (see emit_reloc_geometry): "
            + ", ".join(f"w{w}/e{c}.{i}/m{m}" for w, c, i, m in dropped),
        ]
    body = []
    for (width, ecount, eindex, mapping), lsb in sorted(rows.items()):
