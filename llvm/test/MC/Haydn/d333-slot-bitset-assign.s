...
# Round-trip contract: each distinct op name reappears at least once in the
# disassembly (assignSlotsBitset placed each single-child bundle's op in a
# valid slot, so packInstructionIntoSlot emitted a clean, decodable word).
