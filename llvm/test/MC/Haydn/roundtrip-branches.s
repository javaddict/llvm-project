...
# CHECK: {{.*}}c: 07 0d 66 07 fa 0f 00 00 00 00 00 00{{.*}}bne
# CHECK: {{.*}}18: 07 0d 88 09 f4 0f 00 00 00 00 00 00{{.*}}bge
# CHECK: {{.*}}24: 07 0d ac 0b ee 0f 00 00 00 00 00 00{{.*}}bgeu
# CHECK: {{.*}}30: 07 0d ca 00 e8 0f 00 00 00 00 00 00{{.*}}blt
# CHECK: {{.*}}3c: 07 0d 1e 02 e2 0f 00 00 00 00 00 00{{.*}}bltu
# CHECK: {{.*}}48: 07 0a 38 00 dc 0f 00 00 00 00 00 00{{.*}}beqz
# CHECK: {{.*}}54: 07 0a 4a 00 d6 0f 00 00 00 00 00 00{{.*}}bnez
# CHECK: {{.*}}60: 07 0a 5c 00 d0 0f 00 00 00 00 00 00{{.*}}bgez
# CHECK: {{.*}}6c: 07 0a 6e 00 ca 0f 00 00 00 00 00 00{{.*}}bltz
# CHECK: {{.*}}78: 07 0e 78 00 0c 00 00 00 00 00 00 00{{.*}}jal
# CHECK: {{.*}}84: 07 0d 82 09 06 00 00 00 00 00 00 00{{.*}}jalr
# CHECK-NOT: <unknown>
