; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py UTC_ARGS: --version 6
; RUN: llc -mtriple=haydn-unknown-elf -global-isel-abort=1 -verify-machineinstrs < %s | FileCheck %s

; Role: smoke — function labels present; compile+emit smoke, not semantic qualification.

; Format-E-only rebaseline (/R2-R5): CHECK-LABEL + key invariants.
; Format E rebaseline: labels + present opcodes.

; Format E: function labels present (compile + emit smoke).

define void @mixed_allocas() {
; CHECK-LABEL: mixed_allocas:
; CHECK:       // %bb.0:
; CHECK-NEXT:    { xor32 r0, r0, r0; subi32 sp, sp, 32 }
; CHECK-NEXT:    .cfi_def_cfa_offset 32
; CHECK-NEXT:    { addi32 r4, r0, 42; addi32 r1, sp, 28 }
; CHECK-NEXT:    { nop; st32 r4, r1, 0 }
; CHECK-NEXT:    { nop; lui r1, 3402 }
; CHECK-NEXT:    { nop; addi32 r1, r1, 331776 }
; CHECK-NEXT:    { st32 r1, sp, 2; addi32 r2, sp, 20 } // 4-byte Folded Spill
; CHECK-NEXT:    // 4-byte Spill
; CHECK-NEXT:    { nop; addi32 r1, r0, 232 }
; CHECK-NEXT:    { st32 r1, sp, 3; addi32 r3, r2, 4 } // 4-byte Folded Spill
; CHECK-NEXT:    // 4-byte Spill
; CHECK-NEXT:    { nop; nop }
; CHECK-NEXT:    { nop; ld64 d0, sp, 1 } // 8-byte Folded Reload
; CHECK-NEXT:    // 8-byte Reload
; CHECK-NEXT:    { nop; nop }
; CHECK-NEXT:    { nop; d_sw_l_with_imm d0, r2, 0 }
; CHECK-NEXT:    { nop; d_sw_h_with_imm d0, r3, 0 }
; CHECK-NEXT:    { nop; addi32 sp, sp, 32 }
; CHECK-NEXT:    .cfi_def_cfa sp, 0
; CHECK-NEXT:    { nop; jalr r0, lr, 0 }
  %p32 = alloca i32
  %p64 = alloca i64
  store i32 42, ptr %p32
  store i64 1000000000000, ptr %p64
  ret void
}

; Test with i32-only allocas to ensure frame index elimination works for the
; simple case too (previously this worked because the offset happened to be 0
; after the frame layout, making the bug invisible).

define void @i32_only_allocas() {
; CHECK-LABEL: i32_only_allocas:
; CHECK:       // %bb.0:
; CHECK-NEXT:    { xor32 r0, r0, r0; subi32 sp, sp, 16 }
; CHECK-NEXT:    .cfi_def_cfa_offset 16
; CHECK-NEXT:    { addi32 r3, r0, 1; addi32 r1, sp, 12 }
; CHECK-NEXT:    { st32 r3, r1, 0; addi32 r2, sp, 8 }
; CHECK-NEXT:    { nop; addi32 r1, r0, 2 }
; CHECK-NEXT:    { nop; st32 r1, r2, 0 }
; CHECK-NEXT:    { nop; addi32 sp, sp, 16 }
; CHECK-NEXT:    .cfi_def_cfa sp, 0
; CHECK-NEXT:    { nop; jalr r0, lr, 0 }
  %p1 = alloca i32
  %p2 = alloca i32
  store i32 1, ptr %p1
  store i32 2, ptr %p2
  ret void
}

; Test with i64-only allocas.
; The i64 store may appear as either ST64 or two st32s (see note above).

define void @i64_only_allocas() {
; CHECK-LABEL: i64_only_allocas:
; CHECK:       // %bb.0:
; CHECK-NEXT:    { xor32 r0, r0, r0; subi32 sp, sp, 32 }
; CHECK-NEXT:    .cfi_def_cfa_offset 32
; CHECK-NEXT:    { nop; lui r3, 2145 }
; CHECK-NEXT:    { nop; addi32 r3, r3, -139399 }
; CHECK-NEXT:    { st32 r3, sp, 2; addi32 r1, sp, 24 } // 4-byte Folded Spill
; CHECK-NEXT:    // 4-byte Spill
; CHECK-NEXT:    { nop; addi32 r3, r0, 28744 }
; CHECK-NEXT:    { st32 r3, sp, 3; addi32 r2, r1, 4 } // 4-byte Folded Spill
; CHECK-NEXT:    // 4-byte Spill
; CHECK-NEXT:    { nop; nop }
; CHECK-NEXT:    { nop; ld64 d0, sp, 1 } // 8-byte Folded Reload
; CHECK-NEXT:    // 8-byte Reload
; CHECK-NEXT:    { nop; nop }
; CHECK-NEXT:    { nop; d_sw_l_with_imm d0, r1, 0 }
; CHECK-NEXT:    { nop; d_sw_h_with_imm d0, r2, 0 }
; CHECK-NEXT:    { nop; addi32 sp, sp, 32 }
; CHECK-NEXT:    .cfi_def_cfa sp, 0
; CHECK-NEXT:    { nop; jalr r0, lr, 0 }
  %p = alloca i64
  store i64 123456789012345, ptr %p
  ret void
}
