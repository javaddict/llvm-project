...
; RUN:   -haydn-enable-hwloops < %s 2>&1 | FileCheck %s --check-prefix=HWON
; RUN: llc -mtriple=haydn-unknown-elf -global-isel-abort=1 -filetype=obj < %s \
; RUN:   -o %t.def.o
; RUN: llc -mtriple=haydn-unknown-elf -global-isel-abort=1 \
; RUN:   -haydn-enable-hwloops -filetype=obj -o %t.o < %s
; RUN: llvm-objdump -d -z --triple=haydn-unknown-elf %t.o | \
; RUN:   FileCheck %s --check-prefix=OBJ
; RUN: llvm-readobj -r %t.o | FileCheck %s --check-prefix=RELOC

; Role: object — BEGIN/END displacement QUALIFY while product default
; stays OFF. Immediate hwloop is PC+(uimm<<2); Off1/Off2 must be
; scale-aligned. DEFAULT is the software residual (no SET). HWON arms
; set_hwloop_f2 sel=0 with START/END labels. The HWON obj dump is the
...
; multiples, <<2-legal). HWON asm pins the three SET sites
