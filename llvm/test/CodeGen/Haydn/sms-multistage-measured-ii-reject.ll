...
;
; Same-artifact QUALIFY reject companion — infeasible II must not lie.
;
; A carried add/load chain whose window cannot host a format-exact
; kernel must MultiStageReject / exhaust. It must never sequentialize
; leftovers and still print accepted II=. Product default stays OFF.

; ASM-LABEL: measured_ii_reject:
; ASM: jalr
; RMK-NOT: sequential (preflight)
; RMK: {{exhausted:|rejected:}}
; RMK: qualify-or-cut
; RMK-NOT: accepted II=
; RMK-NOT: qualify parcels-per-iter=
; RMK-NOT: swps observe-only
; RMK-NOT: sequential (preflight)

define i32 @measured_ii_reject(ptr nocapture readonly %a, i32 %n) {
entry:
