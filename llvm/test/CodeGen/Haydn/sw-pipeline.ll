...
; Loop-carried add32 survives. Compare may be SMS-rewritten slt32/bnez or
; the original blt when the Found schedule is single-stage (discarded).
; CHECK:        add32
; CHECK-DAG:    {{(slt32|blt)}}
...
; static-TC shortcut that disposed the kernel must NOT return — blt/slt32
...
; CHECK:        // =>This Inner Loop Header: Depth=1
; add32 survives; runtime compare against %n (blt or SMS slt32/bnez).
; CHECK:        add32
; CHECK-DAG:    {{(slt32|blt)}}
