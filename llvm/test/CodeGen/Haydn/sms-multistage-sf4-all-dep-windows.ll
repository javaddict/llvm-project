...
;
; All-dep-kind placement windows (Data + Anti + Output + memory Order).
; Earliest is no longer Data-only; the LCD remark names the four kinds.
; Product default stays OFF. Hardware loops OFF.
;
; ASM-LABEL: sf4_all_dep:
; ASM: jalr
; RMK: placement-deps=data+anti+output+mem
; RMK: lcd-as-windows
; RMK: {{accepted II=|exhausted:|rejected:}}

define i32 @sf4_all_dep(ptr nocapture readonly %a, i32 %n) {
entry:
