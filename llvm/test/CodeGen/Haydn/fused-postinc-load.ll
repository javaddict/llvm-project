...
; post-increment load instruction (D_LDW_POST / S_LW_POST) when the stride is
...
; port). The backend's D_LDW_POST_INC / S_LW_POST_INC pseudos were expanded to
...
; vec_dot inner loop.
;
; Fix : add real D_LDW_POST / S_LW_POST instruction defs (FmtLSPostInc
...
;
; MIR checks assert the fused instruction appears (D_LDW_POST / S_LW_POST) with
...
; If MIR shows LD64_S1+ADDI32 where D_LDW_POST was expected: the fuse-when
; encodable logic in HaydnExpandPostIncEarly is broken or the D_LDW_POST
...
; Should lower to D_LDW_POST (D_LDW_POST_IMM) — the ae_l64.ip analog.
...
; i32 streaming loop, stride 4 (sizeof(i32)). Should lower to S_LW_POST.
...
loop:
  ...
  ; The post-inc pseudo is D_LDW_POST_INC because LD64_S1 is the i64 load; the
...
; MIR-DAG: {{D_LDW_[A-Z_]*|D_LDW_POST_IMM}}
; MIR: MULA64_LL
...
; The scheduler now issues the extract before the pointer bump; both are
; present and neither depends on the other.
; ASM-DAG: move32_dr_l
; ASM-DAG: addi32{{.*}}, 256
