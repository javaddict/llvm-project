...
; MC serialize-only: refuse sequential E2 split when post-RA packs
; unplaceable multi-member cycles (e.g. dual LS unit collision).
; REQUIRES: haydn-registered-target
;
; Role: object — IR to GlobalISel to MC to ELF to objdump; selected DSP/scalar ops encode and disassemble to expected mnemonics (not printer-only).
; Status: product path; co-issue order free under post-RA packer (CHECK-DAG).
;
; Co-issued bundle members use CHECK-DAG because slot order is free under the
; post-RA packer. Not an asmparser round-trip (llc | llvm-mc).
...
; Objdump may print fmul32s.ll or fmul32s_ll; packer may co-issue ll+lh.
