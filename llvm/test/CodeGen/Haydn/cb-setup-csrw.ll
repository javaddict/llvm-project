...
; `csrw_w <addr>, rs`. This mirrors Hexagon's `m0=rN; cs0=rN` boundary setup.
...
; CHECK:       {{.*}}csrw_w{{(\.s[012])?}} 44, {{r[0-9]+}}{{.*}}
; CHECK:       {{.*}}csrw_w{{(\.s[012])?}} 45, {{r[0-9]+}}{{.*}}
; CHECK:       d_ldw_cb_imm
...
; CHECK:       {{.*}}csrw_w{{(\.s[012])?}} 46, {{r[0-9]+}}{{.*}}
; CHECK:       {{.*}}csrw_w{{(\.s[012])?}} 47, {{r[0-9]+}}{{.*}}
