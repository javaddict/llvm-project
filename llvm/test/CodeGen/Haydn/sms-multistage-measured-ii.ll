; RUN: llc -global-isel-abort=1 -mtriple=haydn -mattr=-hwloop -O2 -verify-machineinstrs \
; RUN:     -haydn-enable-multistage-sms -haydn-multistage-sms-analysis-only \
; RUN:     -pass-remarks-analysis=haydn-multistage-sms < %s \
; RUN:   2>%t.rmk | FileCheck %s --check-prefix=ASM
; RUN: FileCheck %s --check-prefix=RMK < %t.rmk
; RUN: not grep -q 'accepted II=' %t.rmk || FileCheck %s --check-prefix=ACC < %t.rmk
;
; Same-artifact QUALIFY: parcels-per-iteration == searched II.
;
; Bug: ExactCommitPlan[M]==false emitted sequential parcels while
; recordSWPSAnnotation still reported the assumed II (II lie).
; Fix: format infeasibility rejects/retries the II; accept remarks carry
; measured-II equal to the searched II; SWPS annotation is stamped only
; after ParcelsCommitted == II. 2026-08-22 SMS product-default flip rebaseline (default ON).
;
; Analysis-only (hang-containment): materialize is not required to pin
; the remark contract. An accept must print measured-II=N with the same
; N as accepted II= and qualify parcels-per-iter=N searched-II=N.
; Exhaust/reject is fail-closed and also legal here.

; ASM-LABEL: measured_ii:
; ASM: jalr
; RMK-NOT: sequential (preflight)
; RMK: no-seq-fallback
; RMK: resource-bias=slot-windows
; RMK: {{accepted II=|exhausted:|rejected:}}
; RMK: qualify-or-cut
; RMK: product-on
; RMK-NOT: sequential (preflight)
;
; ACC: accepted II=[[II:[0-9]+]]
; ACC-SAME: measured-II=[[II]]
; ACC-SAME: resource-bias={{applied|idle}}
; ACC: qualify parcels-per-iter=[[II]]
; ACC-SAME: searched-II=[[II]]
; ACC: swps observe-only measured-II=[[II]]
; ACC-SAME: searched-II=[[II]] no-asm-stamp

define i32 @measured_ii(ptr nocapture readonly %a, i32 %n) {
entry:
  %cmp = icmp sgt i32 %n, 0
  br i1 %cmp, label %pre, label %exit
pre:
  br label %body
body:
  %i = phi i32 [ %n, %pre ], [ %inext, %body ]
  %s = phi i32 [ 0, %pre ], [ %add, %body ]
  %inext = add nsw i32 %i, -1
  %p = getelementptr inbounds i32, ptr %a, i32 %inext
  %v = load i32, ptr %p, align 4
  %t0 = add i32 %s, %v
  %add = add i32 %t0, 3
  %cond = icmp eq i32 %inext, 0
  br i1 %cond, label %exit, label %body, !llvm.loop !0
exit:
  %r = phi i32 [ 0, %entry ], [ %add, %body ]
  ret i32 %r
}
!0 = distinct !{!0, !1}
!1 = !{!"llvm.loop.itercount.range", i64 16}
