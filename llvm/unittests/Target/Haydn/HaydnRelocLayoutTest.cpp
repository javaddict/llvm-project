...
// authority for PC-rel branch (ValueShift=1, halfword) and hwloop
...
TEST(HaydnRelocLayoutTest, FormatEE2E0FieldLsbParcelOrigin) {
  ...
  EXPECT_EQ(getRelocFieldInfo(RelocKind::WIDE_BranchSImm12).ValueShift, 1u);
}

// WIDE_BranchSImm12: signed 12-bit after ÷2 → byte window [-4096, +4094].
TEST(HaydnRelocLayoutTest, WideBranchDiv2PositiveNegativeBounds) {
  const RelocKind K = RelocKind::WIDE_BranchSImm12;
  const RelocFieldInfo &FI = getRelocFieldInfo(K);
  EXPECT_EQ(FI.ValueShift, 1u);
  EXPECT_EQ(FI.Align, 2u);
  EXPECT_EQ(FI.FieldSize, 12u);
  EXPECT_TRUE(FI.IsSigned);

  EXPECT_TRUE(ok(K, +4094));
  EXPECT_EQ(field(K, +4094), 2047u); // +4094 >> 1
  EXPECT_FALSE(ok(K, +4096));
  EXPECT_STREQ(err(K, +4096), "relocation offset out of range");

  EXPECT_TRUE(ok(K, -4096));
  // Arithmetic shift of -4096 >> 1 = -2048; stored as FieldSize bits.
  EXPECT_EQ(field(K, -4096) & 0xFFFu, 0x800u);
  EXPECT_FALSE(ok(K, -4098));
  EXPECT_STREQ(err(K, -4098), "relocation offset out of range");
...
// BranchSImm16: signed 16-bit after ÷2 → byte window [-65536, +65534].
TEST(HaydnRelocLayoutTest, BranchSImm16Div2PositiveNegativeBounds) {
  ...
  EXPECT_TRUE(ok(K, +65534));
  EXPECT_EQ(field(K, +65534), 32767u);
  EXPECT_FALSE(ok(K, +65536));
  EXPECT_TRUE(ok(K, -65536));
  EXPECT_EQ(field(K, -65536) & 0xFFFFu, 0x8000u);
  EXPECT_FALSE(ok(K, -65538));
}

// CallSImm20: signed 20-bit PC-relative BYTE field (ValueShift=0, Align=1).
// Effective window [-524288, +524287]. Distinct from WIDE_CallSImm20 (÷2).
...
// Effective window [-524288, +524287]. Distinct from WIDE_BranchSImm12 (÷2).
...
// RI12 WIDE branch shares ÷2 / signed-12 geometry with the I12 kind.
// Format E golden places both imm12 fields at parcel bits[32:43] (FieldLsb=32).
TEST(HaydnRelocLayoutTest, WideBranchRISharesDiv2Scale) {
  ...
  EXPECT_TRUE(ok(RelocKind::WIDE_BranchSImm12_RI, +4094));
  EXPECT_FALSE(ok(RelocKind::WIDE_BranchSImm12_RI, +4096));
  EXPECT_TRUE(ok(RelocKind::WIDE_BranchSImm12_RI, -4096));
  EXPECT_FALSE(ok(RelocKind::WIDE_BranchSImm12_RI, -4098));
}

// LongBranchSImm20 (MC-only): same halfword scale as WIDE_CallSImm20.
TEST(HaydnRelocLayoutTest, LongBranchDiv2PositiveNegativeBounds) {
  ...
  EXPECT_TRUE(ok(K, +1048574));
  EXPECT_EQ(field(K, +1048574), 0x7FFFFu);
  EXPECT_FALSE(ok(K, +1048576));
  EXPECT_TRUE(ok(K, -1048576));
  EXPECT_EQ(field(K, -1048576) & 0xFFFFFu, 0x80000u);
  EXPECT_FALSE(ok(K, -1048578));
  EXPECT_FALSE(ok(K, +1));
  EXPECT_STREQ(err(K, +1), "mis-aligned relocation target");
}
...
// Reader undoes ValueShift: branch halfword, call byte, hwloop word addends.
